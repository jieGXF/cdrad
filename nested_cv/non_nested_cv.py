...
from metrics_reports.report_results import ResultReportor
from select_features.feature_selector import FeatureSelector
class NonNestedCV:
    ...
    def __init__(self, model, params_grid, outer_cv=None, inner_cv=None,
                 data_obj=None,feature_selection_methods=None, save_dir=None, subj_column_name=None, label_column_name=None, pat_label=None, con_label=None,
                 document = None,
                 n_jobs=1, cv_options={},
        ...
        self.do_feature_selection = cv_options.get('do_feature_selection',False)
    ...
    def fit(self, X, y):
        ...
        for (i, (train_index, test_index)) in enumerate(outer_cv.split(X, y)):
            ...
            train_set_outer, test_set_outer = X.iloc[train_index], X.iloc[test_index]
            train_set_y_outer, test_set_y_outer = y.iloc[train_index], y.iloc[test_index]

            X_train_outer = train_set_outer.drop([self.subj_column_name, self.label_column_name], axis=1).to_numpy()
            X_test_outer = test_set_outer.drop([self.subj_column_name, self.label_column_name], axis=1).to_numpy()
            y_train_outer = train_set_y_outer.to_numpy()
            y_test_outer = test_set_y_outer.to_numpy()
            ...
            for (j, (train_index_inner, test_index_inner)) in enumerate(inner_cv.split(train_set_outer, train_set_y_outer)):
                log.debug(
                    '\n\t{0}/{1} <-- Current inner fold'.format(j + 1, self.inner_cv))
                train_set_inner, test_set_inner = train_set_outer.iloc[train_index_inner], train_set_outer.iloc[test_index_inner]
                train_set_y_inner, test_set_y_inner = train_set_y_outer.iloc[train_index_inner], train_set_y_outer.iloc[test_index_inner]

                if self.do_feature_selection:
                    save_dir = os.path.join(self.save_dir, "{}_outer_{}_inner".format(i+1, j+1))
                    os.makedirs(save_dir,exist_ok=True)
                    selector = FeatureSelector(self.feature_selection_methods, save_dir, self.subj_column_name,self.label_column_name, self.pat_label, self.con_label)
                    train_set_inner, test_set_inner = selector.run_selection(train_set_inner, test_set_inner, i, j)

                X_train_inner = train_set_inner.drop([self.subj_column_name,self.label_column_name], axis=1).values
                X_test_inner = test_set_inner.drop([self.subj_column_name,self.label_column_name], axis=1).values
                y_train_inner = train_set_y_inner.to_numpy()
                y_test_inner = test_set_y_inner.to_numpy()
                ...
                results = Parallel(n_jobs=self.n_jobs)(delayed(_parallel_fitting)(X_train_inner, X_test_inner, y_train_inner, y_test_inner,param_dict=parameters) for parameters in param_func)
                search_scores.extend(results)  # extend 将列表往后继续追加成一个列表
            ...
            score_train, pred_train = self._predict_and_score(X_train_outer, y_train_outer)
            outer_scores.append(self._transform_score_format(score))

            #######################################
            save_dir_outer = os.path.join(self.save_dir, "{}_outer".format(i))
            os.makedirs(save_dir_outer,exist_ok=True)

            probs = pd.DataFrame(pred_train, columns=["probs"])
            subj = pd.DataFrame(train_set_outer[self.subj_column_name])
            subj.index = range(len(subj))
            label = pd.DataFrame(train_set_outer[self.label_column_name])
            label.index = range(len(label))
            train_probs = pd.concat([subj,label,probs],axis=1)
            train_probs.to_excel(os.path.join(save_dir_outer, 'train_set_classification_result.xlsx'), index=False)

            probs = pd.DataFrame(pred, columns=["probs"])
            subj = pd.DataFrame(test_set_outer[self.subj_column_name])
            subj.index = range(len(subj))
            label = pd.DataFrame(test_set_outer[self.label_column_name])
            label.index = range(len(label))
            test_probs = pd.concat([subj,label,probs],axis=1)
            test_probs.to_excel(os.path.join(save_dir_outer, 'test_set_classification_result.xlsx'), index=False)

            analysis_processor = ResultReportor(test_probs, train_probs, save_dir_outer, self.document, self.subj_column_name, self.label_column_name,self.do_feature_selection)
